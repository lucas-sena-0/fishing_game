/*
 * JOGO DE PESCA - VERSÃO SIMPLIFICADA (ARQUIVO ÚNICO)
 * Para DE1-SoC, roda direto no CPUlator
 */
...
const uint16_t sprite_mina[16] = {
    0x0180, 0x03C0, 0x0FF0, 0x1FF8,
    0x3FFC, 0x7FFE, 0xFFFF, 0xFFFF,
    0xFFFF, 0xFFFF, 0x7FFE, 0x3FFC,
    0x1FF8, 0x0FF0, 0x03C0, 0x0180
};

/* ===== FUNÇÕES BÁSICAS (HARDWARE) ===== */
...
void atualizar_display(int valor) {
    ...
    int d2 = (valor / 100) % 10;
    int d3 = (valor / 1000);
    int hex_code = seg7_table[d0] | (seg7_table[d1] << 8) | (seg7_table[d2] << 16) | (seg7_table[d3] << 24);
...
void desenhar_anzol_skin(int x, int y, uint16_t cor) {
    desenhar_bitmap(x, y, sprite_anzol, 16, 16, cor, 0);
}

/* ===== COLISÕES ===== */
bool verificar_colisao_inimigo(Retangulo player, Inimigo ini) {
    int testX = ini.x, testY = ini.y;
    ...
    int distX = ini.x - testX, distY = ini.y - testY;
    ...
    while (rodando) {
        ...
        for (int i = 0; i < MAX_INIMIGOS; i++) {
            if (inimigos[i].ativo) {
                ...
                inimigos[i].old_ativo[buffer_index] = false;
            }
        }
        ...
        for (int i = 0; i < MAX_PEIXES; i++) {
            if (peixes[i].ativo) {
                ...
                peixes[i].old_ativo[buffer_index] = false;
            }
        }
...
void executar_armazem(void) {
    ...
    desenhar_anzol_skin(85, 100, WHITE);
    desenhar_anzol_skin(150, 100, YELLOW);
    desenhar_anzol_skin(215, 100, ORANGE);
